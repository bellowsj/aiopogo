    ...
    async def user_login(self, username=None, password=None):
        ...
        try:
            ...
            async with ClientSession(
                ...
                async with session.get('https://sso.pokemon.com/sso/logout', params={
                    'service': 'https%3A%2F%2Fsso.pokemon.com%2Fsso%2Foauth2.0%2FcallbackAuthorize'}, proxy=self.proxy,
                                       proxy_auth=self.proxy_auth, allow_redirects=False) as _:
                    pass

                async with session.get('https://sso.pokemon.com/sso/login', params={
                    'service': 'https%3A%2F%2Fsso.pokemon.com%2Fsso%2Foauth2.0%2FcallbackAuthorize',
                    'locale': self.locale}, proxy=self.proxy, proxy_auth=self.proxy_auth) as resp:
                ...
                async with session.post('https://sso.pokemon.com/sso/login',
                                    params={'service': 'http://sso.pokemon.com/sso/oauth2.0/callbackAuthorize',
                                            'locale': self.locale},
                ...
                async with session.post('https://sso.pokemon.com/sso/oauth2.0/accessToken',
                                        headers={'Content-Type': 'application/x-www-form-urlencoded'},
                                        data=token_data, proxy=self.proxy, proxy_auth=self.proxy_auth) as resp:
                    profile_data = {
                ...
                async with session.post('https://sso.pokemon.com/sso/oauth2.0/profile',
                                        headers={'Content-Type': 'application/x-www-form-urlencoded'},
                                        data=profile_data, proxy=self.proxy, proxy_auth=self.proxy_auth) as _:
                    pass
